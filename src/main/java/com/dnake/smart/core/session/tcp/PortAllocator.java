...
public class PortAllocator {
	...
	private static class Record {
		...
		private Record(int port, long happen) {
			this.port = port;
			this.happen = happen;
		}

		private static Record of(int port, long happen) {
			return new Record(port, happen);
		}

		private static Record of(int port) {
			return new Record(port, System.currentTimeMillis());
		}
	}
